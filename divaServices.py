...
class ExecuteOnDivaServices:
    ...
    def main(self, url, input_image, image_path, output_folder):
        ...
        if url is None or input_image is None or output_folder is None:
            raise ValueError("None of the parameter can be None")

        # Upload the Image to DIVAServices
        image_identifier = self.uploadImage(os.path.join(image_path, input_image))
        # Start the binarization process
        resultLink = self.runBinarization(url, image_identifier)
        # Poll for the result
        result = self.pollResult(resultLink)
        # Download the result image
        self.saveFile(result['output'][0]['file']['url'], "bin_" + input_image, output_folder)
    ...
    def saveFile(self, url, filename, output_folder):
